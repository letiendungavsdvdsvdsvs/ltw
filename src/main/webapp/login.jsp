...
String resred = (String) request.getAttribute("resred");
String resSuccess = (String) request.getAttribute("resSuccess");
%>
<!DOCTYPE html>
<html>
<head>
...
<body>
	...
	<div class="container">
		<div class="card w-50 mx-auto my-5">
			...
			<div class="card-body">
			<span>${res!=null ? res : ''}</span>
			<span>${resred!=null ? resred : ''}</span>
			<span>${resSuccess!=null ? resSuccess : ''}</span>
				<form action="user-login" method="post">
					<div class="form-group">
						<label><fmt:message key="label.email-address" /></label> <input
							type="email" name="login-email" class="form-control"
							placeholder="<fmt:message key="label.enter-email"/>">
					</div>
					<div class="form-group">
						<label><fmt:message key="label.password" /></label> <input
							type="password" name="login-password" class="form-control"
							placeholder="<fmt:message key="label.password"/>">
					</div>
					<div>
						<a href="forgotPassword.jsp"><fmt:message key="label.forgot-password"/></a>
					</div>
